<div style="text-align: center; margin: 25vh auto; ">
...
<div class="info" style="width: 40%; margin: 20vh auto;">
    ...
    <span style="width: 70%; float: left;">2026-03-17</span>
</div>
</div>
...
<span style="display: block; text-align: center; margin-top: 10px; color: #888;">图 2.1 系统整体架构设计</span>
...
<span style="display: block; text-align: center; margin-top: 10px; color: #888;">图 3.1 购物车服务时序图</span>
...
<div style="text-align: center; margin: 10px 0; ">
    <img src="../image/homepage.png" alt="首页" width="22%" style="display: inline-block;margin: 0 1%" />
    <img src="../image/cartpage.png" alt="购物车页面" width="22%" style="display: inline-block; margin: 0 1% " />
    <img src="../image/cartpage-empty.png" alt="空购物车页面" width="22%" style="display: inline-block; margin: 0 1%;" />
    <img src="../image/profilepage.png" alt="我的页面" width="22%" style="display: inline-block; margin: 0 1%;" />
    <span style="display: block; margin-top: 5px; color: #888;">图 3.2 各页面设计示例</span>
</div>

#### 3.3.1 首页 HomePage

首页是应用的入口页面，承担商品浏览和分类筛选的核心功能。在课程设计的文档中，要求采用 List 组件实现商品列表展示，但在比较同类应用的设计后，我们发现 Grid 组件更适合展示商品列表，因此在实现中我们采用了 Grid 组件来展示商品列表，而在购物车页面中采用 List 组件展示购物车商品列表，以更好地适应不同的内容展示需求。商品卡片的实现详见工具组件设计部分的 ProductCard 组件。
...
购物车页面负责展示和管理用户添加的商品，支持商品选中、数量控制、删除等操作。正如前文所述，购物车页面采用 List 组件展示购物车商品列表，以更好地适应购物车项的展示需求。购物车项卡片的实现详见工具组件设计部分的 CartItemCard 组件。
...
个人中心页面展示用户信息和常用功能入口。由于此页面绝大部分功能尚未实现，因此点击相关入口时会调用 `Dialog` 工具显示相关提示，详见后文工具组件设计部分。本页面具体设计如下：
...
入口组件负责管理底部导航和页面切换，通过维护一个 `currentIndex` 状态来控制当前显示的页面。底部导航栏组件 `BottomNavBar` 负责显示三个 tab 和购物车角标，并通过回调函数通知父组件切换页面。
...
#### 3.4.5 对话框工具 Dialog
<div style="text-align: center; margin: 0 5%; float: right; width: 22%; ">
    <img src="../image/dialog-undo.png" alt="未实现功能弹窗提示" width="100%" />
    <span style="display: block; margin-top: 5px; color: #888;">图 3.3 功能尚未实现提示示例</span>
</div>
...
由于本项目采用本地模拟数据实现，部分功能（如订单结算、消息通知等）尚未实现，点击相关入口时会调用 `Dialog.showNotImplemented()` 显示提示。
